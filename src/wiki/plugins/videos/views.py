...
from wiki.plugins.videos import forms, models
from wiki.views.mixins import ArticleMixin

logger = logging.getLogger(__name__)


class VideoView(ArticleMixin, ListView):
    ...
    def get_queryset(self):
        if (self.article.can_moderate(self.request.user) or
            ...
            videos = models.Video.objects.filter(article=self.article)
        else:
...
class DeleteView(ArticleMixin, RedirectView):
    ...
    def dispatch(self, request, article, *args, **kwargs):
        self.video = get_object_or_404(models.Video, article=article,
    ...
    def get_redirect_url(self, **kwargs):
        ...
        if not self.video.current_revision:
            logger.critical('Encountered an image without current revision set, ID: {}'.format(self.video.id))
        ...
        new_revision.inherit_predecessor(self.video)
        new_revision.set_from_request(self.request)
        new_revision.revision_number = RevisionPluginRevision.objects.filter(plugin=self.video).count()
...
class PurgeView(ArticleMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        return redirect('wiki:videos_index', article_id=self.article_id)
